...
func demultiplexingFunc(dataSourceChan chan int, amount int) (chan bool, []chan int) {
...
func multiplexingFunc(exit chan bool, channels ...chan int) <-chan int {
	var wg sync.WaitGroup
...
func main() {
	...
	exit, consumers := demultiplexingFunc(startDataSource(), 5)
	c := multiplexingFunc(exit,consumers...)
